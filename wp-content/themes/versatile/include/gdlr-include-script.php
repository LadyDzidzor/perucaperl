	...
	if( !function_exists('gdlr_regiser_use_script') ){
		function gdlr_regiser_use_script($array){	
			...
			if( !empty($theme_option['new-fontawesome']) && $theme_option['new-fontawesome'] == 'enable' ){
				$array['style']['font-awesome'] = GDLR_PATH . '/plugins/font-awesome-new/css/font-awesome.min.css';
			}else{
			...
			if( is_search() || is_archive() || 
				...
				$array['style']['versatile-flexslider'] = GDLR_PATH . '/plugins/flexslider/flexslider.css';
				$array['script']['versatile-flexslider'] = GDLR_PATH . '/plugins/flexslider/jquery.flexslider.js';
			}
			
			// nivoslider
			if( is_page() && gdlr_search_page_builder($all_page_builder, 'item', 'slider', array('slider-type', 'nivoslider')) ){
