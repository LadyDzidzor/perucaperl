<?php
	...
	*/	
	...
	add_filter('embed_oembed_html', 'gdlr_oembed_cookies');
	if( !function_exists('gdlr_oembed_cookies') ){
		function gdlr_oembed_cookies( $return ){
			global $theme_option;
			$theme_option['disable-cookie-youtube'] = empty($theme_option['disable-cookie-youtube'])? 'disable': $theme_option['disable-cookie-youtube'];

			if( $theme_option['disable-cookie-youtube'] == 'enable' ){
	...
	if( !function_exists('gdlr_get_video') ){
		function gdlr_get_video($video, $size = 'full'){
			...
			// youtube link
				...
				return '<iframe src="//www.youtube' . ($theme_option['disable-cookie-youtube'] == 'enable'? '-nocookie': '') . '.com/embed/' . $id[1] . '?wmode=transparent' . $id[2] . '" width="' . $width . '" height="' . $height . '" ></iframe>';
			
			// youtu.be link
			}else if(strpos($video, 'youtu.be') !== false){
				preg_match('#youtu.be\/([^?&]+)#', $video, $id);
				return '<iframe src="//www.youtube' . ($theme_option['disable-cookie-youtube'] == 'enable'? '-nocookie': '') . '.com/embed/' . $id[1] . '?wmode=transparent" width="' . $width . '" height="' . $height . '" ></iframe>';
		...
		function gdlr_get_post_slider_item( $settings ){
			...
			while($query->have_posts()){ $query->the_post();
				...
				if( !empty($image_id) ){
					...
					if( !empty($settings['style']) && $settings['style'] == 'no-excerpt' ){
	...
	if( !function_exists('gdlr_get_flex_slider') ){
		function gdlr_get_flex_slider($slides, $settings = array()){
			...
			$ret .= empty($settings['pausetime'])? 'data-pausetime="' . esc_attr($theme_option['flex-pause-time']) . '" ': 
						'data-pausetime="' . esc_attr($settings['pausetime']) . '" ';
			$ret .= empty($settings['slidespeed'])? 'data-slidespeed="' . esc_attr($theme_option['flex-slide-speed']) . '" ': 
						'data-slidespeed="' . esc_attr($settings['slidespeed']) . '" ';			
			$ret .= empty($settings['effect'])? 'data-effect="' . esc_attr($theme_option['flex-slider-effects']) . '" ': 
			...
			foreach($settings['slider'] as $slide_id => $slide){
				if( ($current_size >= ($paged - 1) * $settings['num-fetch']) &&
					...
					if($settings['show-caption'] != 'no'){
						$gal_caption = gdlr_get_attachment_info($slide_id, 'caption');
						if( !empty($gal_caption) ){
							$ret .= '<span class="gallery-caption">' . $gal_caption . '</span>';
						}
					}
